...
void santaChain(int y, int x, int dir, int turn){
    ...
    map[y][x] = 0;
    if(oob(y+dy[dir] * D, x + dx[dir] * D)) santa[curSantaNum].isOut = true;
    else if(map[y+dy[dir] * D][x + dx[dir] * D] >= 1){
        santaSantaChain(y+dy[dir] * D,x + dx[dir] * D,dir);
        map[y+dy[dir] * D][x + dx[dir] * D] = curSantaNum;
    }
    else map[y + dy[dir] * D][x + dx[dir] * D] = curSantaNum;
